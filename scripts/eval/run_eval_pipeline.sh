...
# LLM check -- source dual LLM config if not already set
if [[ -z "${USE_LOCAL_LLM:-}" && -z "${OPENAI_API_KEY:-}" && -z "${OPENAI_KEY:-}" ]]; then
    if [[ -f "$PROJECT_ROOT/export_dual_llm.sh" ]]; then
        log "Sourcing export_dual_llm.sh for LLM configuration"
        source "$PROJECT_ROOT/export_dual_llm.sh"
    else
        fail "No LLM configured. Run: source export_dual_llm.sh"
    fi
fi
echo "LLM env: OK (USE_LOCAL_LLM=${USE_LOCAL_LLM:-unset})"
...
echo "  python scripts/eval/evaluate.py --configs $CONFIGS"
